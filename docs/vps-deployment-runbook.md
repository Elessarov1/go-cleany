...
### Восстановление старого backup с Rental-фотографиями

Одноразовая миграция существующих Rental-фотографий завершена, и startup-backfill удалён из
приложения. Новые загрузки всегда создают full/card/thumbnail варианты. После восстановления старого
backup проверьте nullable variant-поля до открытия сервиса клиентам:
...
Ненулевой результат не ломает fallback на исходный asset, но такие карточки снова будут отдавать
полноразмерное изображение. Перед полноценным использованием восстановленной базы потребуется
отдельный контролируемый repair responsive-вариантов; обычный запуск backend его больше не выполняет.
...
Google ↔ Telegram linking использует одноразовый bot handoff. После свежего Google-входа страница
`/account` создаёт link challenge и открывает `https://t.me/<TELEGRAM_BOT_USERNAME>?start=link_<token>`.
Пользователь подтверждает Telegram identity сообщением боту, а web-страница завершает привязку своим
polling. Отдельный Mini App deep link и отдельная страница подтверждения больше не требуются.
...
10. администратор задаёт тариф Transfer, создаёт phone-only водителя и вручную назначает заявку;
11. после настройки Telegram ID одноразовая ссылка подключает правильного водителя, а при
    `DRIVER_SELF_ACCEPT` два одновременных принятия не могут назначить двух водителей.
