# Локальный запуск go-cleany и go-renty в Docker
...
Параметры `RENTAL_*` задают общую политику go-renty: минимальный и максимальный срок, скидку за
...
7. если текущий Telegram ID указан в `ADMIN_TELEGRAM_IDS`, бот должен прислать администратору сообщение о каждой созданной брони;
...
В разделах администрирования go-renty доступен персональный переключатель Telegram-уведомлений о
бронях. Он изменяет настройку только текущего администратора и хранится в PostgreSQL. Для него не
нужна новая переменная окружения: список потенциальных получателей по-прежнему задаёт
`ADMIN_TELEGRAM_IDS`, а отсутствие персональной строки означает, что уведомления включены.
