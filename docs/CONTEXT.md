...
Stage 7.5 adds a local-only measurement foundation before optimization. The isolated `loco-perf` Compose project provides a deterministic scalable dataset, pinned Docker k6 scenarios, JFR capture and performance-only Actuator/Hibernate metrics. Production remains health-only. Smart Reminders, data retention and Rental Cleaning Benefit issuance share tagged scheduler counters and structured per-run logs without changing their schedules or business semantics. Frequent source builds on the small VPS are bounded operationally by pruning unused BuildKit cache older than a configurable retention window before a deploy and dangling images after a successful health check; runtime containers, volumes, database data and backups are never part of that cleanup. The next measurement MR records the unchanged baseline before Rental media, frontend boot or scheduler/runtime optimizations are selected.
