...
export class MockAnalyticsApi implements AnalyticsApi {
  ...
  getOverview(request: AnalyticsOverviewRequest): Promise<AnalyticsOverview> {
    ...
    return Promise.resolve({
      ...
      rentalSearch: all || request.service === "RENTAL" ? {
        total: {
          searchExecutions: 120,
          zeroResultSearches: 18,
          openedSearches: 74,
          createdBookingSearches: 21,
          completedBookingSearches: 14,
          conflictSearches: 4,
          zeroResultRate: 0.15,
          openRate: 0.6167,
          creationRate: 0.175,
          completionRate: 0.1167,
          conflictRate: 0.0333,
          medianApiDurationMs: 82,
          medianFirstCardDurationMs: 190,
          medianHoursToBooking: 5.2,
        },
        byMode: (["DATE_RANGE", "MONTHLY", "BROWSE_ALL"] as const).map((mode, index) => ({
          mode,
          funnel: {
            searchExecutions: 50 - index * 10,
            zeroResultSearches: mode === "BROWSE_ALL" ? 0 : 8 - index * 2,
            openedSearches: 31 - index * 5,
            createdBookingSearches: 9 - index * 2,
            completedBookingSearches: 6 - index,
            conflictSearches: index,
            zeroResultRate: mode === "BROWSE_ALL" ? 0 : 0.16,
            openRate: 0.62,
            creationRate: 0.18,
            completionRate: 0.12,
            conflictRate: index / (50 - index * 10),
            medianApiDurationMs: 78 + index * 8,
            medianFirstCardDurationMs: 185 + index * 10,
            medianHoursToBooking: 4.8 + index,
          },
        })),
      } : {
        total: {
          searchExecutions: 0, zeroResultSearches: 0, openedSearches: 0,
          createdBookingSearches: 0, completedBookingSearches: 0, conflictSearches: 0,
          zeroResultRate: null, openRate: null, creationRate: null,
          completionRate: null, conflictRate: null, medianApiDurationMs: null,
          medianFirstCardDurationMs: null, medianHoursToBooking: null,
        },
        byMode: [],
      },
      repeatActions: [
